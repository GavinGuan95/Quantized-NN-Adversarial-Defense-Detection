...
# when someone uses gradient_overwrite_map and names this QuantizeGrad as the value
# the key function
...
class Linear(Layer):
    ...
    def __init__(self, num_hid, detail, dropout_rate=0.5):
    ...
    def fprop(self, x, reuse):
        ...
        with tf.variable_scope(self.detail + self.name + '_fprop', reuse):
            x = tf.nn.dropout(x, rate=self.dropout_rate)  # there is a dropout rate that should be independent of phase
...
class Conv2DQuantize(Layer):

  def __init__(self, binary, output_channels, kernel_shape, strides, padding, phase, scope_name, bit):
    self.__dict__.update(locals())
    self.G = tf.get_default_graph()
    del self.self

  def my_tf_quant(self, x, bit):
    scale = 2**(bit-1)
    with self.G.gradient_override_map({"ClipByValue": "QuantizeGrad", "Round": "QuantizeGrad"}):
      return tf.clip_by_value(tf.round(x * scale, name="Round") / scale, -1.0, 1.0, name="ClipByValue")
  ...
  def set_input_shape(self, input_shape, reuse):
    ...
    with tf.variable_scope(self.scope_name + '_init', reuse):
      ...
      if self.binary:
        self.kernels = self.my_tf_quant(self.kernels, self.bit)
        k_bin_summ = tf.summary.histogram(
          name="k_bin", values=self.kernels)
    ...
    with tf.variable_scope(self.scope_name + '_fprop', reuse=reuse) as scope:
      if self.binary:
        ...
        x = tf.contrib.layers.batch_norm(
          x, epsilon=BN_EPSILON, is_training=self.phase,
          reuse=reuse, scope=scope)
        x = self.my_tf_quant(x, self.bit)
      x = tf.nn.conv2d(x, self.kernels, (1,) + tuple(self.strides) + (1,), self.padding)
      a_u, a_v = tf.nn.moments(tf.abs(x), axes=[0], keep_dims=False)
      a_summ = tf.summary.histogram('a', values=x)
      a_u_summ = tf.summary.scalar("a_u", tf.reduce_mean(a_u))
      a_v_summ = tf.summary.scalar("a_v", tf.reduce_mean(a_v))
...
def make_basic_cnn(phase, temperature, detail, nb_filters=64, nb_classes=10, input_shape=(None, 28, 28, 1), dropout=0.0):
    layers = [Conv2D(False, nb_filters, (3, 3), (1, 1), "VALID", phase, detail + 'conv1'),
              ReLU(),
              Conv2D(False, nb_filters, (3, 3), (1, 1), "VALID", phase, detail + 'conv2'),
              ReLU(),
              # tf.nn.max_pool(ksize=(2, 2)),
              Flatten(),
              Linear(256, detail, dropout),
              ReLU(),
              Linear(nb_classes, detail, dropout),
              Softmax(temperature)]
...
def make_scaled_rand_cnn(phase, temperature, detail, nb_filters=64, nb_classes=10, input_shape=(None, 28, 28, 1), dropout=0.0):
    layers = [Conv2D(False, nb_filters, (8, 8), (2, 2), "SAME", phase, detail + 'conv1'),
              ReLU(),
              Conv2D(False, nb_filters * 2, (6, 6), (2, 2), "VALID", phase, detail + 'conv2'),
              ReLU(),
              Conv2DRand(False, nb_filters * 2, (5, 5), (1, 1), "VALID", phase, detail + 'conv3'),
              SReLU(detail + 'srelu3_fp'),
              Flatten(),
              Linear(128, detail, dropout),
              ReLU(),
              Linear(nb_classes, detail, dropout),
              Softmax(temperature)]
...
def make_basic_binary_cnn(phase, temperature, detail, nb_filters=64, nb_classes=10, input_shape=(None, 28, 28, 1), dropout=0.0):
    print("in make_basic_binary_cnn")
    layers = [Conv2D(False, nb_filters, (3, 3), (1, 1), "VALID", phase, detail + 'conv1'),
              ReLU(),
              Conv2D(True, nb_filters, (3, 3), (1, 1), "VALID", phase, detail + 'conv2_bin'),
              ReLU(),
              Conv2D(True, nb_filters, (3, 3), (1, 1), "VALID", phase, detail + 'conv3_bin'),
              ReLU(),
              Flatten(),
              Linear(256, detail, dropout),
              ReLU(),
...
def make_quantized_cnn(phase, temperature, detail, nb_filters=64, nb_classes=10, input_shape=(None, 28, 28, 1), dropout=0.0, bit=2):
  print("in make_basic_binary_cnn")
  layers = [Conv2D(False, nb_filters, (3, 3), (1, 1), "VALID", phase, detail + 'conv1'),
            ReLU(),
            Conv2DQuantize(True, nb_filters, (3, 3), (1, 1), "VALID", phase, detail + 'conv2_bin', bit=bit),
            ReLU(),
            Conv2DQuantize(True, nb_filters, (3, 3), (1, 1), "VALID", phase, detail + 'conv3_bin', bit=bit),
            ReLU(),
            Flatten(),
            Linear(256, detail, dropout),
            ReLU(),
...
def make_scaled_binary_cnn(phase, temperature, detail, nb_filters=64, nb_classes=10, input_shape=(None, 28, 28, 1), dropout=0.0):
    print("in make_scaled_binary_cnn")
    layers = [Conv2D(False, nb_filters, (8, 8), (2, 2), "SAME", phase, detail + 'conv1'),
              ReLU(),
              Conv2D(True, nb_filters * 2, (6, 6), (2, 2), "VALID", phase, detail + 'conv2_bin'),
              SReLU(detail + 'srelu2_bin'),
              Conv2D(True, nb_filters * 2, (5, 5), (1, 1), "VALID", phase, detail + 'conv3_bin'),
...
def make_scaled_binary_rand_cnn(phase, logits_scalar, detail, nb_filters=64, nb_classes=10, input_shape=(None, 28, 28, 1), dropout=0.0):
  print("make_scaled_binary_rand_cnn")
  layers = [Conv2D(False, nb_filters, (3, 3), (1, 1), "VALID", phase, detail + 'conv1'),
            ReLU(),
            Conv2D(True, nb_filters, (3, 3), (1, 1), "VALID", phase, detail + 'conv2_bin'),
            SReLU(detail + 'srelu2_bin'),
            Conv2DRand(True, nb_filters, (3, 3), (1, 1), "VALID", phase, detail + 'conv3_bin'),
            SReLU(detail + 'srelu3_bin'),
            Flatten(),
            Linear(256, detail, dropout),
            ReLU(),
  ...
  model = MLP(layers, input_shape)
  print('Finished making basic binary cnn kernel 3')
  return model
