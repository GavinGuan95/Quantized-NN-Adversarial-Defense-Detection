...
class Dropout(Layer):
    ...
    def fprop(self, x, reuse):
        return tf.nn.dropout(x, rate=self.dropout_rate) # always dropout 0.3, whether training or testing
...
######################### full-precision #########################
def make_basic_cnnV0(phase, temperature, detail, nb_filters=64, nb_classes=10, input_shape=(None, 28, 28, 1)):
    # def __init__(self, binary, output_channels, kernel_shape, strides, padding, phase, scope_name):
    layers = [Conv2D(False, nb_filters, (8, 8), (2, 2), "SAME", phase, detail + 'conv1'),
              ReLU(),
              Conv2D(False, nb_filters * 2, (6, 6), (2, 2), "VALID", phase, detail + 'conv2'),
              ReLU(),
              Conv2D(False, nb_filters * 2, (5, 5), (1, 1), "VALID", phase, detail + 'conv3'),
              ReLU(),
              Flatten(),
              Linear(128, detail),
              ReLU(),
...
def make_basic_cnn(phase, temperature, detail, nb_filters=64, nb_classes=10, input_shape=(None, 28, 28, 1)):
...
def make_basic_binary_cnnV0(phase, temperature, detail, nb_filters=64, nb_classes=10,
                          input_shape=(None, 28, 28, 1)):
...
def make_scaled_binary_rand_cnn(phase, logits_scalar, detail, nb_filters=64, nb_classes=10,
    ...
    layers = [Conv2D(False, nb_filters, (8, 8), (2, 2), "SAME", phase, detail + 'conv1'),
              ...
              Conv2D(True, nb_filters * 2, (6, 6), (2, 2), "VALID", phase, detail + 'conv2_bin'),
              SReLU(detail + 'srelu2_bin'),
              Conv2DRand(True, nb_filters * 2, (5, 5), (1, 1), "VALID", phase, detail + 'conv3_bin'),
              SReLU(detail + 'srelu3_bin'),
              Flatten(),
              Linear(128, detail),
              ReLU(),
...
# def make_binary_rand_cnn(phase, temperature, detail, nb_filters=64, nb_classes=10,
#                           input_shape=(None, 28, 28, 1)):
#     print("in make_binary_rand_cnn")
#     layers = [Conv2D(False, nb_filters, (8, 8), (2, 2), "SAME", phase, detail + 'conv1'),
#               ReLU(),
#               Conv2D(True, nb_filters * 2, (6, 6), (2, 2), "VALID", phase, detail + 'conv2_bin'),
#               ReLU(),
