...
def model_eval_multi_run(sess, x, y, predictions=None, X_test=None, Y_test=None, phase=None, writer=None,
               feed=None, args=None, model=None, repeat_run=50):
    """
    Compute the accuracy of a TF model on some data
    :param sess: TF session to use when training the graph
    :param x: input placeholder
    ...
    :param args: dict or argparse `Namespace` object.
                 Should contain `batch_size`
    :param model: (deprecated) if not None, holds model output predictions
    :param repeat_run: number of times to run through the model for uncertainty estimation
    :return: a float with the accuracy value
    """
    args = _FlagsWrapper(args or {})
    ...
    if model is None and predictions is None:
        raise ValueError("One of model argument predictions argument must be supplied.")
    if model is not None:
        warnings.warn("model argument is deprecated. Switch to predictions argument. model argument will be removed after 2018-01-05.")
        if predictions is None:
            predictions = model
        else:
            raise ValueError("Exactly one of model argument and predictions argument should be specified.")
    ...
    with sess.as_default():
        ...
        nb_batches = int(math.ceil(float(len(X_test)) / args.batch_size))
        assert nb_batches * args.batch_size >= len(X_test)
        all_mean_var = np.array([])
        all_square_mean_var = np.array([])
        for batch in range(nb_batches):
            if batch % 100 == 0 and batch > 0:
                _logger.debug("Batch " + str(batch))
            ...
            # The last batch may be smaller than all others, so we need to
            # account for variable batch size here
            feed_dict = {x: X_test[start:end], y: Y_test[start:end], phase: False}
            if feed is not None:
                feed_dict.update(feed)
            ...
            if writer is not None:
                ...
                # cur_acc = acc_value.eval(feed_dict=feed_dict) # Gavin commented out
                # Gavin new added
                all_pred = []
                for i in range(repeat_run):
                    cur_acc, cur_correct_pred, cur_pred = sess.run([acc_value, correct_preds, predictions], feed_dict=feed_dict)
                    # cur_acc: float containing the accuracy
                    # cur_correct_pred: ndarray(1d), vector of bool, size is the current batch size, True means the prediction is correct
                    # cur_pred: ndarray(2d), first index is each test instance (128 in total), second index is the logit for each class (10 in total)
                    # Need to have a option to perform multiple runs and get all the predictions and get the uncertainty
                    all_pred.append(cur_pred)
                all_pred_np = np.asarray(all_pred)
                var_each_element = all_pred_np.var(axis=0)
                mean_var_of_each_input = var_each_element.mean(axis=1)
                square_mean_var_of_each_input = np.square(var_each_element).mean(axis=1)
                group_pred_sum = np.sum(all_pred_np, axis=0)
                group_pred_result = np.argmax(group_pred_sum, axis=1)
                pred_truth = np.argmax(Y_test[start:end], axis=1)
                group_right_or_wrong = np.array(group_pred_result == pred_truth, dtype=int)
                group_acc = np.mean(group_right_or_wrong)


            group_accuracy += (cur_batch_size * group_acc)
            accuracy += (cur_batch_size * cur_acc)  # For now, the accuracy is only measured by the last repeat_run
            all_mean_var = np.concatenate((all_mean_var, mean_var_of_each_input), axis=0)
            all_square_mean_var = np.concatenate((all_square_mean_var, square_mean_var_of_each_input), axis=0)

        assert end >= len(X_test)

        # Divide by number of examples to get final value
        accuracy /= len(X_test)
        group_accuracy /= len(X_test)

    return accuracy, all_mean_var, all_square_mean_var, group_accuracy
